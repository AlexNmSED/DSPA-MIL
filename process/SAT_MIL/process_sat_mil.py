...
from modules.SAT_MIL.sat_mil import SAT_MIL
...
def process_SAT_MIL(args):
    print_args(args)

    train_dataset = WSI_Dataset_GS(args.Dataset.dataset_csv_path,'train')
    val_dataset = WSI_Dataset_GS(args.Dataset.dataset_csv_path,'val')
    test_dataset = WSI_Dataset_GS(args.Dataset.dataset_csv_path,'test')
    ...
    mil_model = SAT_MIL(num_classes=num_classes,embed_dim=in_dim,num_heads=num_heads)
    mil_model.to(device)
    ...
    for epoch in tqdm(range(args.General.num_epochs),colour='GREEN'):
        ...
        train_loss,cost_time = train_loop_SAT(device,mil_model,train_dataloader,criterion,optimizer,now_scheduler)
        val_loss,val_metrics = val_loop_SAT(device,num_classes,mil_model,val_dataloader,criterion)
        if args.Dataset.VIST == True:
            test_loss,test_metrics = val_loss,val_metrics
        else:
            test_loss,test_metrics = val_loop_SAT(device,num_classes,mil_model,test_dataloader,criterion)
        FAIL = '\033[91m'
        ENDC = '\033[0m'
        print('----------------INFO----------------\n')
