...
def val_loop(device,num_classes,model,loader,criterion,retrun_WSI_feature = False,return_WSI_attn=True):
    model.eval()
    val_loss_log = 0
    labels = []
...
def train_loop_SAT(device,model,loader,criterion,optimizer,scheduler):
    
    start = time.time()
    model.train()
    train_loss_log = 0
    gs_criterion = torch.nn.CrossEntropyLoss()
    for i, data in enumerate(loader):
        optimizer.zero_grad()
        label = data[1].long().to(device)

        gleason_3_label = data[2].long().to(device)
        gleason_4_label = data[3].long().to(device)
        gleason_5_label = data[4].long().to(device)

        bag = data[0].to(device).float()
        isup_logits, g3_logits, g4_logits, g5_logits = model(bag)

        isup_train_loss = criterion(isup_logits, label)

        g3_train_loss = gs_criterion(g3_logits, gleason_3_label)
        g4_train_loss = gs_criterion(g4_logits, gleason_4_label)
        g5_train_loss = gs_criterion(g5_logits, gleason_5_label)

        # 计算三个分类头损失的均值
        gs_mean_loss = (g3_train_loss + g4_train_loss + g5_train_loss) / 3

        # 整体损失 = 平均分类损失 + 融合分类头损失
        alpha = 0.3  # 分类头损失的权重
        train_loss = alpha*gs_mean_loss + isup_train_loss
...
def val_loop_SAT(device,num_classes,model,loader,criterion,retrun_WSI_feature = False,return_WSI_attn=False):
    model.eval()
    val_loss_log = 0
    labels = []
    WSI_features = []
    ...
    with torch.no_grad():
        for i, data in enumerate(loader):
            ...
            val_logits, g3_logits, g4_logits, g5_logits, tok1, tok2, tok3 = forward_return['fused_logits'], forward_return['val1'], forward_return['val2'], forward_return['val3'], forward_return['inp_cls_token_1'], forward_return['inp_cls_token_2'], forward_return['inp_cls_token_3']
            ...
            isup_val_loss = criterion(val_logits,label)

            g3_val_loss = gs_criterion(g3_logits, gleason_3_label)
            g4_val_loss = gs_criterion(g4_logits, gleason_4_label)
            g5_val_loss = gs_criterion(g5_logits, gleason_5_label)

            # 计算三个分类头损失的均值
            gs_val_mean_loss = (g3_val_loss + g4_val_loss + g5_val_loss) / 3

            # 整体损失 = 平均分类损失 + 融合分类头损失
            alpha = 0.1  # 分类头损失的权重
            val_loss = alpha*gs_val_mean_loss + isup_val_loss
...
def test_loop_SAT(device,num_classes,model,loader,criterion,retrun_WSI_feature = False,return_WSI_attn=False):
    model.eval()
    val_loss_log = 0
    labels = []
    WSI_features = []
    ...
    with torch.no_grad():
        for i, data in enumerate(loader):
            ...
            val_logits, g3_logits, g4_logits, g5_logits, tok1, tok2, tok3 = model(bag)

            token1_feats.append(tok1.squeeze(0).cpu().numpy())
            token2_feats.append(tok2.squeeze(0).cpu().numpy())
            token3_feats.append(tok3.squeeze(0).cpu().numpy())
    ...
    output_dict = {
        'token1': np.stack(token1_feats),
        'token2': np.stack(token2_feats),
        'token3': np.stack(token3_feats),
        'labels': np.concatenate(labels)
    }

    with open('/n/archive00/labs/IT/GDC/xihao/HIPT-master-org/2-Weakly-Supervised-Subtyping/MIL_BASELINE-main/token_features_testset.pkl', 'wb') as f:
...
def dgr_train_loop(device,model,loader,criterion,optimizer,scheduler,now_epoch,epoch_des,n_lesion):
    ...
    for i, data in enumerate(loader):
        ...
        if now_epoch < epoch_des:
            ...
            div_loss = -torch.logdet(lesion_norm@lesion_norm.T+1e-10*torch.eye(n_lesion, device=device))
...
def clam_val_loop(device,num_classes,model,loader,criterion,bag_weight, pkl_path=None, retrun_WSI_feature = False,return_WSI_attn=False):
    model.eval()
    val_loss_log = 0
    labels = []
    bag_predictions_after_normal = []
    ...
    if pkl_path:
        # 将预测概率值和标签保存到指定的 pkl 文件中
        with open(pkl_path, 'wb') as f:
            pickle.dump((bag_predictions_after_normal, labels), f)
...
def ds_train_loop(device,model,loader,criterion,optimizer,scheduler):
    ...
    for i, data in enumerate(loader):
        ...
        loss_max = criterion(max_prediction, label.float())
