...
def test(args):
    ...
    if yaml_args.General.MODEL_NAME == 'DTFD_MIL':
        ...
        mil_model.load_state_dict(torch.load(model_weight_path, map_location='cuda'))

    pkl_path = os.path.join(args.test_log_dir,f'Test_dataset_{yaml_args.General.MODEL_NAME}_{args.test_data_name}_{args.test_fold}.pkl')
    ...
    if yaml_args.General.MODEL_NAME == 'CLAM_MB_MIL':
        ...
        test_loss,test_metrics = clam_val_loop(device,num_classes,mil_model,test_dataloader,criterion,bag_weight,pkl_path=pkl_path)
    ...
    test_log_path = os.path.join(test_log_dir,f'Test_Log_{model_name}_{test_data_name}_{test_fold}.txt')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test_data_name',type=str,default='TCGA-PRAD',help='data name for test')
    parser.add_argument('--test_fold',type=str,default='fold_1',help='train model_weight fold')
